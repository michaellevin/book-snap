...
def download(pth, download=True):
    ...
    if download:
        ...
        for i, im in enumerate(all_images):
            im_address = "https://content.prlib.ru/fcgi-bin/iipsrv.fcgi?FIF=/var/data/scans/public/{}/{}/{}&JTL=2,0&CVT=JPEG".format(
                *ids, im
            )
            print("{}/{}  >  ".format(i, len(all_images)), im_address)
            system_call(
                "{} -l {} {}.jpeg".format(
                    DEZOOMIFY_PTH, im_address, join(dest_folder, str(i).zfill(3))
                )
            )
